...
pub async fn list(pool: Data<DBPool>) -> HttpResponse {
    ...
    let tweets_with_likes = Tweets {
        results: tweets
            ...
            .map(|tweet| {
                let _likes =
                    list_likes(Uuid::parse_str(tweet.id.as_str()).unwrap(), &conn).unwrap();
                tweet.add_likes(_likes.results)
            })
            .collect::<Vec<Tweet>>(),
    };
...
#[post("/tweets")]
pub async fn create(pool: Data<DBPool>, tweet_req: Json<TweetRequest>) -> HttpResponse {
    let conn = pool.get().expect(CONNECTION_POOL_ERROR);
...
#[get("/tweets/{id}")]
pub async fn get(pool: Data<DBPool>, path: Path<String>) -> HttpResponse {
    let conn = pool.get().expect(CONNECTION_POOL_ERROR);
    let tweet =
        web::block(move || find_tweet(Uuid::parse_str(path.0.as_str()).unwrap(), &conn)).await;

    let conn = pool.get().expect(CONNECTION_POOL_ERROR);
    match tweet {
        Ok(tweet) => {
            let _likes = list_likes(Uuid::parse_str(tweet.id.as_str()).unwrap(), &conn).unwrap();
...
#[delete("/tweets/{id}")]
pub async fn delete(pool: Data<DBPool>, path: Path<String>) -> HttpResponse {
    let conn = pool.get().expect(CONNECTION_POOL_ERROR);
    let _ =
        web::block(move || delete_tweet(Uuid::parse_str(path.0.as_str()).unwrap(), &conn)).await;
